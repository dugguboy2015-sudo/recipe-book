...
- Acceptance:
  - [x] Migration trial-run in a rolled-back transaction with three fake households (curator, owner, outsider): founding claim 34 rows; a pending recipe saved through the wrapper was owned and pending; anon saw 31 (unchanged) and not the pending one, nor did `recipe_stats`; the owner and the curator saw it; the outsider didn't. Rollback left 0 households, 0 users, no function
  - [x] After apply: `anon` REST still returns 31 recipes; production smoke green; `save_household_recipe` service-role-only, `current_household_is_curator` authenticated-only, neither `security definer`; policies `recipes_public_read {anon}` and `recipes_member_read {authenticated}`
  - [x] 20/20 end-to-end checks on the preview with three throwaway members (one temporarily flagged curator): member create → 201 pending and owned; anon, and another household, can't see it; its own household and the curator can; another household's delete → 404; non-curator approve → 403; curator approve → public and anon-visible; another household's edit → 403 `not_your_recipe`, restore → 403; owner edit/delete/restore → 200; five audit rows all carry the acting member's user id; quota 5 for a member, `requiresSignIn` for anon; anon generate and bad-token writes → 401
  - [x] `npm run smoke -- --write` passed against the preview — the first time the write smoke has run against a real deployment rather than local dev (it now signs in as a throwaway member instead of relying on Turnstile's local test keys)
  - [x] Browser, preview: signed out, recipe cards show no Edit/Delete, the + button opens "Sign in to add recipes" instead of the form, the AI panel explains the per-household quota; signed in as a member with a pending recipe, the pending notice appears, "Show them" filters to `?status=pending` showing the "Awaiting approval" badge and Edit on the member's own recipe
  - [x] All test users, households and `__smoke__` rows removed (0 households, 0 auth users, 0 smoke recipes)
  - [x] `npm run check` green (343 tests; 13 new permission tests, 39 existing Function tests moved from Turnstile to signed-in members)
- Deviations from spec / findings while building this:
  1. **Turnstile was removed from the write and AI path rather than "skipped for signed-in users"** — since every write now requires a member, there is no anonymous write left for it to protect. It moved to the sign-in email instead (the scarcest resource: 2 emails/hour project-wide), but **Supabase's enforcement of it is deliberately left off**: it gates the founding owner's first sign-in, and this environment can't obtain a real Turnstile token (verified: the same timeout happens on production's old code), so it can't be proven end to end here. The client sends the token and fails open; switching enforcement on is one dashboard setting, documented in `docs/operations.md`.
  2. **Rules live once, in `public/js/shared/permissions.js`**, imported by the Functions (which enforce) and the browser (which only decides which buttons to show).
